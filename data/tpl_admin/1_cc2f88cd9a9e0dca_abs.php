	...
	<div class="content">
		...
		<tr>
			<td><select name="option_list" id="option_list">
					...
					<optgroup label="表单">
						<?php $opt_list_id["num"] = 0;$opt_list=is_array($opt_list) ? $opt_list : array();$opt_list_id["total"] = count($opt_list);$opt_list_id["index"] = -1;foreach($opt_list AS $key=>$value){ $opt_list_id["num"]++;$opt_list_id["index"]++; ?>
					...
					<?php if($title_list){ ?>
					<optgroup label="主题">
						<?php $title_list_id["num"] = 0;$title_list=is_array($title_list) ? $title_list : array();$title_list_id["total"] = count($title_list);$title_list_id["index"] = -1;foreach($title_list AS $key=>$value){ $title_list_id["num"]++;$title_list_id["index"]++; ?>
