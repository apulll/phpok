...
function save()
{
	...
	$("#modify").ajaxSubmit({
		'url':get_url('ext','save','id=<?php echo $module;?><?php if($tid){ ?>&tid=<?php echo $tid;?><?php } ?><?php if($tmpid){ ?>&tmpid=<?php echo $tmpid;?><?php } ?>'),
		'type':'post',
		'dataType':'json',
		'success':function(rs){
			if(rs.status == 'ok')
...
<div class="table">
	...
	<div class="content">
		<table>
		<tr>
			<td><select id="form_type" name="form_type" onchange="_phpok_form_opt(this.value,'form_type_ext','<?php echo $id;?>')">
...
$(document).ready(function(){
	_phpok_form_opt("<?php echo $rs['form_type'];?>","form_type_ext","<?php echo $rs['id'];?>");
});
</script>
