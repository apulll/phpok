...
class content_control extends phpok_control
{
	...
	public function index_f()
	{
		...
		if(!$rs['project_id']){
			error(P_Lang('未绑定项目'),$this->url,5);
		}
		if(!$rs['module_id']){
			error(P_Lang('未绑定相应的模块'));
		}
		...
		if($rs['cate_id'] && $project['cate']){
			...
			if(!$cate_root_rs || !$cate_root_rs['status']){
				error(P_Lang('根分类信息不存在或未启用'),$this->url,5);
			}
			...
			if(!$cate_rs || !$cate_rs['status']){
				error(P_Lang('分类信息不存在或未启用'),$this->url,5);
			}
			if($cate_rs['parent_id']){
				...
				if(!$cate_parent_rs || !$cate_root_rs['status']){
					error(P_Lang('父级分类信息不存在或未启用'),$this->url,5);
				}
